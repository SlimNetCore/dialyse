# `ng-table` — documentation complète

Table Angular Material configurable, standalone, basée sur les signals (`input()`/`output()`/`computed()`). Sélecteur : `<ng-table>`, classe `NgTableComponent`.

Fichiers : `ng-table.component.ts` / `.html` / `.css`, `ng-table-labels.ts`, `column-filter-renderer.component.*`, `select-filter.component.*`, `dynamic-filter-host.component.ts` (les 3 derniers sont des détails d'implémentation internes, non exportés).

## Dépendances

**npm (peer dependencies)** : `@angular/core`, `@angular/common`, `@angular/material` (modules `button`, `checkbox`, `icon`, `menu`, `table`, `tooltip`, `form-field`, `input`, `select`, `datepicker`, `core`), `rxjs`. Rien d'autre — pas de `@ngx-translate` ni aucune autre dépendance i18n/métier.

**Configuration requise côté consommateur** : aucune. Le composant fonctionne dès l'installation, avec un style et des textes par défaut (français) — voir les sections [Personnaliser les textes](#personnaliser-les-textes-ngtablelabels) et [Personnaliser le style](#personnaliser-le-style-variables-css) pour aller plus loin, mais ce n'est jamais obligatoire.

## Rôle

`ng-table` centralise tout ce qu'une table de données a besoin de faire, réutilisable dans n'importe quel projet Angular :

- colonnes configurables : visibilité, ordre (drag-and-drop), largeur (resize)
- tri, filtrage (menu ou inline), barre de filtres actifs
...
- deux modes de données : `local` (tout côté client) ou `remote` (délégué au serveur)
- responsive (colonne d'actions condensée sous 760px)
...
@Component({
  ...
  template: `<ng-table [columns]="columns" [rows]="rows()" [rowSelectionEnabled]="true" />`,
})
export class MyListComponent {
  readonly rows = signal<MyRow[]>([...]);

  readonly columns: NgTableColumn<MyRow>[] = [
    {id: 'code', header: 'Code', valueAccessor: (r) => r.code, sortable: true, copy: true},
    {id: 'nom', header: 'Nom', valueAccessor: (r) => r.nom, sortable: true, filter: {type: 'text'}},
    {
      id: 'statut',
      header: 'Statut',
      valueAccessor: (r) => r.statut,
      filter: {type: 'enum', options: [{value: 'ACTIF', label: 'Actif'}, {value: 'INACTIF', label: 'Inactif'}]},
    },
  ];
}
```
...
`optionsLoader` accepte un `Observable` ou une `Promise`. Pendant le chargement, un spinner s'affiche dans le menu ; en cas d'erreur ou de liste vide, un message s'affiche (personnalisable via `labels.refOptionsLoading`/`refOptionsEmpty`/`refOptionsLoadError`, voir [Étape 19](#étape-19--personnaliser-les-textes-et-linternationalisation)).
...
Liste complète des 14 variables (bordures, fonds, texte, ombre...) : voir [Personnaliser le style](#personnaliser-le-style-variables-css) plus bas.
...
| `labels`                    | `Partial<NgTableLabels>`                                         | `{}`      | Textes à surcharger (voir [Personnaliser les textes](#personnaliser-les-textes-ngtablelabels)).                      |
...
| `dataMode`                  | `'local' \| 'remote'`                                            | `'local'` | Voir [Mode local / distant](#mode-local--distant).                                                                   |
| `pageTrackingEnabled`       | `boolean`                                                        | `false`   | Voir [Pagination](#pagination).                                                                                      |
...
Aucune dépendance i18n : tous les textes de l'UI (boutons, aria-labels, messages) sont en dur (français par défaut) et overridables via `[labels]`, en ne fournissant que ce que vous voulez changer :
...
```html
<ng-table [labels]="{resetFiltersButton: 'Clear filters', noData: 'No results'}" ... />
```

**Pour une vraie internationalisation** (plusieurs langues), calculez `[labels]` dans un `computed()` qui dépend de votre service i18n (ex. `@ngx-translate/core`) et se recalcule au changement de langue :
...
tableLabels = computed<Partial<NgTableLabels>>(() => {
  ...
  return {
    ...
    // ... etc, filterBy: this.translate.instant('COMMON.FILTER_BY', {field: '{field}'})
  };
});
```
...
- Les filtres texte sont **debouncés à 350 ms** (via `groupBy` + `mergeMap(debounceTime)`, un flux indépendant par colonne) avant de committer et déclencher `filtersChange`/`remoteQueryChange` — évite une requête par caractère tapé en mode `remote`. Les filtres à choix fixe (select/enum/boolean/date) committent immédiatement.

...
public class AuthRestController {
    ...
    @Value("${app.auth.cookie.refresh-path:/api/v1/auth/refresh}")
    ...
    public ResponseEntity<LogoutResponse> logout(HttpServletRequest request) {
        ...
        ResponseCookie expiredAccess = buildCookie(authCookieName, "", 0, authCookiePath);
        ResponseCookie expiredRefresh = buildCookie(refreshCookieName, "", 0, refreshCookiePath);
        return ResponseEntity.ok()
    ...
    public ResponseEntity<RefreshResponse> refresh(HttpServletRequest request) {
        ...
        try {
            ...
            ResponseCookie accessCookie = buildCookie(authCookieName, newAccessToken, authCookieMaxAgeSec, authCookiePath);
            ResponseCookie refreshCookie = buildCookie(refreshCookieName, newRefreshToken, refreshCookieMaxAgeSec, refreshCookiePath);
    ...
    private ResponseCookie buildCookie(String cookieName, String tokenValue, long maxAgeSec, String path) {

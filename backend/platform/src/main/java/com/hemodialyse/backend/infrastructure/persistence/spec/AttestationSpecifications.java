...
public final class AttestationSpecifications {

    private AttestationSpecifications() {
    }

    public static Specification<AttestationJpaEntity> from(AttestationSearchRequest req) {
        return (root, query, cb) -> {
            ...
            if (req.search() != null && !req.search().isBlank()) {
                predicates.add(patientGlobalLike(root, query, cb, req.search()));
            }

            DateRange debut = parseFlexibleDateRange(req.debut());
            if (debut != null) {
                if (debut.from != null) predicates.add(cb.greaterThanOrEqualTo(root.get("dateDebut"), debut.from));
                if (debut.to != null) predicates.add(cb.lessThanOrEqualTo(root.get("dateDebut"), debut.to));
            }

            DateRange fin = parseFlexibleDateRange(req.fin());
            if (fin != null) {
                if (fin.from != null) predicates.add(cb.greaterThanOrEqualTo(root.get("dateFin"), fin.from));
                if (fin.to != null) predicates.add(cb.lessThanOrEqualTo(root.get("dateFin"), fin.to));
            }
    ...
    private static Predicate patientFieldLike(Root<AttestationJpaEntity> root,
    ...
    private static Predicate patientNomPrenomLike(Root<AttestationJpaEntity> root,
                                                  jakarta.persistence.criteria.CriteriaQuery<?> query,
                                                  jakarta.persistence.criteria.CriteriaBuilder cb,
                                                  String value) {
        String term = likeTerm(value);
    ...
    private static Predicate patientGlobalLike(Root<AttestationJpaEntity> root,
                                               jakarta.persistence.criteria.CriteriaQuery<?> query,
                                               jakarta.persistence.criteria.CriteriaBuilder cb,
                                               String value) {
        String term = likeTerm(value);

...
public class FactureReglementJdbcAdapter implements ReglementRepository {
    ...
    public PagedResult<ReglementUseCase.ReglementFactureListItem> search(ReglementUseCase.ReglementSearchQuery query) {
        ...
        List<ReglementUseCase.ReglementFactureListItem> items = jdbc.query(
                ...
                (rs, rowNum) -> mapListItem(
                        rs.getObject("facture_id", UUID.class),
                        rs.getString("numero_facture"),
                        rs.getString("numero_assurance"),
                        rs.getString("patient_nom"),
                        rs.getString("patient_prenom"),
                        rs.getObject("caisse_id", UUID.class),
                        rs.getString("caisse_nom"),
                        rs.getObject("agence_id", UUID.class),
                        rs.getString("agence_nom"),
                        rs.getObject("centre_payeur_id", UUID.class),
                        rs.getString("centre_payeur_nom"),
                        rs.getObject("date_facturation", LocalDate.class),
                        rs.getBigDecimal("montant_facture"),
                        rs.getBigDecimal("montant_regle")
                ),
                args.toArray()
        );
    ...
    public ReglementUseCase.ReglementDashboardResult dashboard(ReglementUseCase.ReglementDashboardQuery query) {
        ...
        List<ReglementUseCase.ReglementDashboardBucket> buckets = List.of(
                new ReglementUseCase.ReglementDashboardBucket(
                        ReglementUseCase.FactureReglementEtat.NON_REGLEE.name(),
                        "Non reglee",
                        nonReglees,
                        sum(items.stream()
                                .filter(item -> item.etat() == ReglementUseCase.FactureReglementEtat.NON_REGLEE)
                                .map(ReglementUseCase.ReglementFactureListItem::montantFacture)
                                .toList())
                ),
                new ReglementUseCase.ReglementDashboardBucket(
                        ReglementUseCase.FactureReglementEtat.PARTIELLEMENT_REGLEE.name(),
                        "Partiellement reglee",
                        partiellementReglees,
                        sum(items.stream()
                                .filter(item -> item.etat() == ReglementUseCase.FactureReglementEtat.PARTIELLEMENT_REGLEE)
                                .map(ReglementUseCase.ReglementFactureListItem::montantFacture)
                                .toList())
                ),
                new ReglementUseCase.ReglementDashboardBucket(
                        ReglementUseCase.FactureSoldeType.REGLE.name(),
                        "Reglee",
                        items.stream().filter(item -> item.soldeType() == ReglementUseCase.FactureSoldeType.REGLE).count(),
                        sum(items.stream()
                                .filter(item -> item.soldeType() == ReglementUseCase.FactureSoldeType.REGLE)
                                .map(ReglementUseCase.ReglementFactureListItem::montantFacture)
                                .toList())
                ),
                new ReglementUseCase.ReglementDashboardBucket(
                        ReglementUseCase.FactureSoldeType.TROP_PERCU.name(),
                        "Trop percu",
                        tropPercus,
                        totalTropPercu
                )
        ...
        return new ReglementUseCase.ReglementDashboardResult(
    ...
    public FactureReglementAggregate loadAggregate(CenterId centerId, UUID factureId) {
        BigDecimal montantFacture = jdbc.query(
                        ...
                        (rs, rowNum) -> rs.getBigDecimal("total_ttc"),
                        factureId,
                        centerId.value())
                .stream()
                .findFirst()
        ...
        List<FacturePayment> payments = jdbc.query(
                "SELECT id, facture_id, center_id, montant, date_reglement, saisi_par FROM facture_reglements WHERE facture_id = ? AND center_id = ? ORDER BY date_reglement, created_at, id",
    ...
    public List<ReglementUseCase.FacturePaymentItem> getPaymentHistory(CenterId centerId, UUID factureId) {
        return jdbc.query(
                "SELECT id, facture_id, montant, date_reglement, saisi_par " +
    ...
    private SqlFilterParts buildFilter(CenterId centerId,
                                       LocalDate start,
                                       LocalDate end,
                                       UUID caisseId,
                                       UUID agenceId,
    ...
    private ReglementUseCase.ReglementFactureListItem mapListItem(UUID factureId,
        ...
        BigDecimal facture = scale(montantFacture);
        BigDecimal regle = scale(montantRegle);
        FactureReglementStatusSpecification.Evaluation evaluation = FactureReglementStatusSpecification.evaluate(facture, regle);
        return new ReglementUseCase.ReglementFactureListItem(
                ...
                facture,
                regle,
                scale(evaluation.reste()),
                scale(evaluation.tropPercu()),
                evaluation.etat(),
                evaluation.soldeType()
        );
    }
